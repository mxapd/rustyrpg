...
pub fn attack(attacker: &mut dyn Combatant, defender: &mut dyn Combatant) {
    let mut damage =
        rand::thread_rng().gen_range((attacker.calc_damage() / 2)..=attacker.calc_damage());
...
pub fn fight(player: &mut Character, enemy: &mut Enemy) {
    ...
    while fight_ongoing {
        ...
        let selection: u8 = selection_string
            .trim()
            .parse()
            ...
            5 => {
                if attempt_flee(player.evasion) {
                    ...
                    let damage = rand::thread_rng()
                        .gen_range((enemy.calc_damage() / 2)..=enemy.calc_damage());
                    player.take_damage(damage);
                    println!(
                        "{} attacks you for {} damage while you fail to escape.",
                        enemy.name(),
                        damage
                    );
